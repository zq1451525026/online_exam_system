                        ...
                        <div class="panel panel-primary">
                            ...
                            <div class="panel-body">
                                <table class="table table-striped"  id="head">
                                    ...
                                    <tbody>
                                    <c:forEach var="knowledge" items="${selectKnowledgeList.getResult()}">
                                    ...
                                    <tr>
                                        <td colspan="5" style="text-align: center">
                                            <input type="hidden" name="tech_ctgr" value="selectKnowledgeList.getTech_ctgr()">
                                            <br>
                                            第${selectKnowledgeList.getPageNum()}页/共${selectKnowledgeList.getPages()}页&nbsp;&nbsp;
                                            <c:if test="${selectKnowledgeList.getPageNum() gt 1}">
                                                <a href="knowledge/selectKnowledge?pageNum=1">第一页</a>&nbsp;&nbsp;
                                            </c:if>
                                            <c:if test="${selectKnowledgeList.getPageNum() gt 1}">
                                                <a href="knowledge/selectKnowledge?pageNum=${selectKnowledgeList.getPageNum()-1}">上一页</a>&nbsp;&nbsp;
                                            </c:if>
                                            <c:if test="${selectKnowledgeList.getPageNum() lt selectKnowledgeList.getPages()}">
                                                <a href="knowledge/selectKnowledge?pageNum=${selectKnowledgeList.getPageNum()+1}">下一页</a>&nbsp;&nbsp;
                                            </c:if>
                                            <c:if test="${selectKnowledgeList.getPageNum() lt selectKnowledgeList.getPages()}">
                                                <a href="knowledge/selectKnowledge?pageNum=${selectKnowledgeList.getPages()}">末页</a>&nbsp;&nbsp;
                                            </c:if>
                                            总记录数：${selectKnowledgeList.getTotal()}
                                        </td>
                                    </tr>
                                    </tbody>
                                </table>
...
<script>
    ...
    /*function toSave(c) {
        ...
        $.ajax({
            type:"post",
            url:"knowledge/selectKnowledge",
            data:"tech_ctgr="+c,
            success:function (msg) {
                if (msg == "0"){
                    document.getElementById("tech_ctgr").value = false;
                }else {
                    document.getElementById("tech_ctgr").value = true;
                }
            },
            error:function (msg) {
                alert("发送失败，请重新发送！");
